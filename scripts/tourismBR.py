...
st.sidebar.success(  'Highest visitors year: {} with {} arrivals.'   .format(DF.groupby('year')['arrivals'].sum().idxmax(), f"{DF.groupby('year')['arrivals'].sum().max():,.0f}"))
st.sidebar.divider(                          )
st.sidebar.markdown('''
...
ax =plt.subplot(111)
ax =sns.barplot(     y='arrivals'     ,        x=df.index,  data=df, hue=values,   palette='Greens_r'      ,        saturation=.75,     legend=False )
